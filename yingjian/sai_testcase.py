...
class TestCase:
    filepath = r'./'
    sndname =  'SaiAEC.wav'   # 'snd' corresponds to Send Signal to SPK
    rcvname =  'SaiAEC_海尔1.wav'   # 'rcv' corresponds to Receive Signal at MIC ARRAY
    micseq = [1,2,3,4,5,6,7]
    refseq = [8]
    init_t = 15
    ...
    def _data_split(self,micseqlag,refseqlag):
        ...
        '''
        print('mic_ref_lag',(refseqlag.argmax() - micseqlag.argmax())/16,'ms')
        ...
        self.sndmusic = self.snddata[45*self.fs:73*self.fs,:]        
        self.micmusic = self.micdata[miclag+45*self.fs:miclag+73*self.fs,:]
        self.refmusic = self.refdata[reflag+45*self.fs:reflag+73*self.fs,:]
        self.sndnoise = self.snddata[75*self.fs:105*self.fs,:]        
        self.micnoise = self.micdata[miclag+75*self.fs:miclag+105*self.fs,:]
        self.refnoise = self.refdata[reflag+75*self.fs:reflag+105*self.fs,:]
        self.sndchirp2 = self.snddata[107*self.fs:127*self.fs,:]        
        self.micchirp2 = self.micdata[miclag+107*self.fs:miclag+127*self.fs,:]
        self.refchirp2 = self.refdata[reflag+107*self.fs:reflag+127*self.fs,:]
